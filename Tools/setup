...
function install {
    ...
    if checkifinstalled; then
        ...
        # create CredentialCacheFolder folder
        mkdir -p /usr/share/$db_dir/CredentialCacheFolder
        chown www-data:www-data /usr/share/CredentialCacheFolder
        cd $install_dir/
        $install_dir/$app --seed
        cd ${BASEDIR}
        chown www-data:www-data /usr/share/$db_dir/askstatus.db
        chmod 644 /usr/share/$db_dir/askstatus.db
